...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadRecentViewedMovies(showAll = false) {
        ...
        for (let i = 0; i < itemsToShow; i++) {
            ...
            const movieFavicon = getFaviconUrl(movie.link);
            const websiteDisplayName = movie.website || getDomainFromUrl(movie.link);
            const websiteTag = websiteDisplayName ? `<span class="category-genre-tag category-website-tag"><img class="website-tag-favicon" src="${escapeHtml(movieFavicon)}" alt="" onerror="this.style.display='none'">${escapeHtml(websiteDisplayName)}</span>` : '';
        ...
        try {
            ...
            if (data.success && data.sections) {
                ...
                for (const [sectionKey, sectionData] of Object.entries(data.sections)) {
                    // Skip built-in sections that don't need dynamic fetching
                    const upperKey = sectionKey.toUpperCase();
                    if (upperKey === 'SEARCH_LINKS' || upperKey === 'MOVIE_COLLECTIONS') {
                        continue;
                    }
    ...
    function updateWebsiteResult(websiteName, data) {
        ...
        if (data.success) {
            ...
            const headerHTML = `
                <div class="website-header">
                    <span class="website-name">
                        <img class="source-favicon" src="${escapeHtml(faviconUrl)}" alt="" onerror="this.style.display='none'">
                        ${escapeHtml(websiteName)}
                    </span>
                    <a href="search-results.php?query=${encodeURIComponent(currentSearchQuery)}&website=${encodeURIComponent(data.website)}" class="more-btn" aria-label="See All ${escapeHtml(websiteName)} results">
            ...
            if (data.count === 0) {
                ...
                if (data.hasMore) {
                    carouselHTML += `
                        <div class="carousel-item-wrapper">
                            <div class="trending-next-card" id="${nextCardId}" data-website="${escapeHtml(websiteName)}">
                                <span class="trending-next-card-text">Next</span>
                            </div>
                        </div>
                    `;
                ...
                // Prefetch next page to show actual next movie preview
                if (data.hasMore) {
                    const actualNextPage = (data.page || 1) + 1;
                    prefetchSearchNextCard(websiteName, currentSearchQuery, actualNextPage);
                }
            }
        } else {
    ...
    // Prefetch search next page to show actual next movie preview
    async function prefetchSearchNextCard(websiteName, query, nextPage) {
        const nextCard = document.getElementById(`search-next-${websiteName}`);
        if (!nextCard) return;

        // Set immediate fallback from current results while fetching
        const paginationInfo = searchPaginationData[websiteName];
        if (paginationInfo && paginationInfo.results && paginationInfo.results.length > 0) {
            const lastResult = paginationInfo.results[paginationInfo.results.length - 1];
            if (lastResult && lastResult.image) {
                nextCard.style.backgroundImage = `url('${lastResult.image}')`;
                nextCard.style.backgroundSize = 'cover';
                nextCard.style.backgroundPosition = 'center';
            }
        }

        // Store request context for verification after fetch
        const targetPage = nextPage;
        const requestQuery = query;

        try {
            const response = await fetch(`search-single.php?query=${encodeURIComponent(query)}&website=${encodeURIComponent(websiteName)}&page=${nextPage}`);
            const data = await response.json();

            // Verify the next card still exists, page hasn't advanced, and query hasn't changed
            const currentNextCard = document.getElementById(`search-next-${websiteName}`);
            const currentPaginationInfo = searchPaginationData[websiteName];
            const currentPage = currentPaginationInfo ? currentPaginationInfo.page : 1;

            // Only update if card exists, query matches, and we're targeting the correct next page
            if (currentNextCard && currentSearchQuery === requestQuery && currentPage + 1 === targetPage) {
                if (data.success && data.results && data.results.length > 0) {
                    const nextMovie = data.results[0];
                    if (nextMovie.image) {
                        currentNextCard.style.backgroundImage = `url('${nextMovie.image}')`;
                        currentNextCard.style.backgroundSize = 'cover';
                        currentNextCard.style.backgroundPosition = 'center';
                    }
                }
            }
        } catch (error) {
            console.error(`Error prefetching search next card for ${websiteName}:`, error);
        }
    }
        ...
        try {
            ...
            if (data.success && data.count > 0) {
                ...
                if (data.hasMore) {
                    const nextCardHtml = `
                        <div class="carousel-item-wrapper">
                            <div class="trending-next-card" id="search-next-${websiteName}" data-website="${escapeHtml(websiteName)}">
                                <span class="trending-next-card-text">Next</span>
                            </div>
                        </div>
                    `;
                    ...
                    // Prefetch next page to show actual next movie preview
                    const actualNextPage = (data.page || nextPage) + 1;
                    prefetchSearchNextCard(websiteName, currentSearchQuery, actualNextPage);
                }
    ...
    const genericSectionNextPagePreview = {}; // Store prefetched next page first movie for preview
                ...
                if (sectionData.count > 0) {
                    ...
                    sectionData.results.forEach(movie => {
                        ...
                        if (!genericSectionLoadedIds[sectionName].has(movieId)) {
                            ...
                            const itemHtml = `
                                <div class="carousel-item-wrapper">
                                    <div class="carousel-item new-item" data-title="${escapeHtml(movie.title)}" data-link="${escapeHtml(movie.link)}" data-image="${escapeHtml(movie.image || '')}" data-language="${escapeHtml(movie.language || '')}" data-genre="${escapeHtml(movie.genre || '')}" data-imdb="${escapeHtml(movie.imdb || '')}" data-website="${escapeHtml(movie.website || '')}">
                                        ${movie.image ? `<img src="${escapeHtml(movie.image)}" alt="${escapeHtml(movie.title)}" class="result-image lazy-image" loading="lazy" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22200%22 height=%22280%22%3E%3Crect fill=%22%23ddd%22 width=%22200%22 height=%22280%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 dominant-baseline=%22middle%22 text-anchor=%22middle%22 fill=%22%23999%22 font-size=%2220%22%3ENo Image%3C/text%3E%3C/svg%3E'">` : '<div class="result-image"></div>'}
                                    </div>
                                    <div class="carousel-item-title" title="${escapeHtml(movie.title)}">${escapeHtml(movie.title)}</div>
                                </div>
    ...
    function displayCategoryMovies(categoryName, movies, append = false) {
        ...
        if (append) {
            ...
            if (grid) {
                ...
                movies.forEach((movie, index) => {
                    ...
                    const catMovieFavicon = getFaviconUrl(movie.link);
                    const catWebsiteName = movie.website || getDomainFromUrl(movie.link);
                    const websiteTag = catWebsiteName ? `<span class="category-genre-tag category-website-tag"><img class="website-tag-favicon" src="${escapeHtml(catMovieFavicon)}" alt="" onerror="this.style.display='none'">${escapeHtml(catWebsiteName)}</span>` : '';

                    const itemHtml = `
                        <div class="category-grid-item new-item" data-title="${escapeHtml(movie.title)}" data-link="${escapeHtml(movie.link)}" data-image="${escapeHtml(movie.image || '')}" data-language="${escapeHtml(movie.language || '')}" data-genre="${escapeHtml(movie.genre || '')}" data-imdb="${escapeHtml(movie.imdb || '')}" data-website="${escapeHtml(movie.website || '')}">
                            ${movie.image ? `<img src="${escapeHtml(movie.image)}" alt="${escapeHtml(movie.title)}" class="result-image lazy-image" loading="lazy" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22200%22 height=%22280%22%3E%3Crect fill=%22%23ddd%22 width=%22200%22 height=%22280%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 dominant-baseline=%22middle%22 text-anchor=%22middle%22 fill=%22%23999%22 font-size=%2220%22%3ENo Image%3C/text%3E%3C/svg%3E'">` : '<div class="result-image"></div>'}
                            <div class="category-movie-info">
                                <h3 class="category-movie-title">${escapeHtml(movie.title)}</h3>
        ...
        movies.forEach((movie, index) => {
            ...
            const categoryTag = movie.category ? `<span class="category-genre-tag">${escapeHtml(movie.category)}</span>` : '';
            const gridMovieFavicon = getFaviconUrl(movie.link);
            const gridWebsiteName = movie.website || getDomainFromUrl(movie.link);
            const websiteTag = gridWebsiteName ? `<span class="category-genre-tag category-website-tag"><img class="website-tag-favicon" src="${escapeHtml(gridMovieFavicon)}" alt="" onerror="this.style.display='none'">${escapeHtml(gridWebsiteName)}</span>` : '';
    ...
    // Prefetches next page to show actual next movie preview
    async function loadGenericSectionNextCardBackground(sectionKey) {
        const nextCard = document.getElementById(`${sectionKey}NextCard`);

        if (!nextCard) return;

        // Set immediate fallback from current section data while fetching
        const currentSectionData = genericSectionDataStore[sectionKey] || [];
        if (currentSectionData.length > 0) {
            const lastMovie = currentSectionData[currentSectionData.length - 1];
            if (lastMovie && lastMovie.image) {
                nextCard.style.backgroundImage = `url(${lastMovie.image})`;
                nextCard.style.backgroundSize = 'cover';
                nextCard.style.backgroundPosition = 'center';
            }
        }

        // Get current page from store (this is the page that was LAST loaded)
        const currentPage = genericSectionPages[sectionKey] || 1;
        const nextPage = currentPage + 1;

        // Store target page to verify after fetch
        const targetPage = nextPage;

        try {
            // Prefetch next page to get actual next movie
            const response = await fetch(`deduplicated-sections.php?page=${nextPage}&section=${sectionKey}`);
            const data = await response.json();

            // Verify the next card still exists and page hasn't advanced
            const currentNextCard = document.getElementById(`${sectionKey}NextCard`);
            const currentStoredPage = genericSectionPages[sectionKey] || 1;

            // Only update if card exists and we're still targeting the same next page
            if (currentNextCard && currentStoredPage + 1 === targetPage) {
                if (data.success && data.sections && data.sections[sectionKey]) {
                    const sectionData = data.sections[sectionKey];
                    if (sectionData.results && sectionData.results.length > 0) {
                        // Get the first movie from next page (actual next movie)
                        const nextMovie = sectionData.results[0];
                        genericSectionNextPagePreview[sectionKey] = { movie: nextMovie, forPage: targetPage };
            ...
            console.error(`Error prefetching next page for ${sectionKey}:`, error);
        }
    }
    ...
    function displayWeeklyTop10(movies) {
        ...
        movies.forEach((movie, index) => {
            ...
            carouselHTML += `
                <div class="carousel-item-wrapper">
                    <div class="carousel-item" data-title="${escapeHtml(movie.title)}" data-link="${escapeHtml(movie.link)}" data-image="${escapeHtml(movie.image || '')}" data-language="${escapeHtml(movie.language || '')}" data-website="${escapeHtml(movie.website || '')}" data-rank="${rank}">
                        ${movie.image ? `<img src="${escapeHtml(movie.image)}" alt="${escapeHtml(movie.title)}" class="result-image lazy-image" loading="lazy" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22200%22 height=%22280%22%3E%3Crect fill=%22%23ddd%22 width=%22200%22 height=%22280%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 dominant-baseline=%22middle%22 text-anchor=%22middle%22 fill=%22%23999%22 font-size=%2220%22%3ENo Image%3C/text%3E%3C/svg%3E'">` : '<div class="result-image"></div>'}
...
(function() {
    function initCollectionsCarousel() {
        ...
        const prevBtn = document.querySelector('.carousel-btn-prev[data-carousel="collectionsTrack"]');
        const nextBtn = document.querySelector('.carousel-btn-next[data-carousel="collectionsTrack"]');

        if (!collectionsTrack || !prevBtn || !nextBtn) return;

        function updateArrowsVisibility() {
            const scrollLeft = collectionsTrack.scrollLeft;
            const maxScroll = collectionsTrack.scrollWidth - collectionsTrack.clientWidth;
        ...
        updateArrowsVisibility();
        collectionsTrack.addEventListener('scroll', updateArrowsVisibility);

        prevBtn.addEventListener('click', function() {
            const scrollAmount = collectionsTrack.offsetWidth * 0.8;
        ...
        const nextCard = document.getElementById('collectionsNextCard');
        if (nextCard && window.COLLECTIONS_DATA && window.COLLECTIONS_PER_PAGE) {
    ...
    function loadMoreCollections() {
        ...
        if (!collectionsTrack || !window.COLLECTIONS_DATA || typeof window.COLLECTIONS_CURRENT_INDEX === 'undefined') return;

        const perPage = window.COLLECTIONS_PER_PAGE || 4;
        const currentIndex = window.COLLECTIONS_CURRENT_INDEX;
        const allCollections = window.COLLECTIONS_DATA;
        ...
        const nextBatch = allCollections.slice(currentIndex, currentIndex + perPage);
        
        nextBatch.forEach((collection, idx) => {
            const cardHTML = createCollectionCardHTML(collection, currentIndex + idx);
            if (nextCard) {
                nextCard.insertAdjacentHTML('beforebegin', cardHTML);
            } else {
                collectionsTrack.insertAdjacentHTML('beforeend', cardHTML);
            }
        });

        window.COLLECTIONS_CURRENT_INDEX = currentIndex + nextBatch.length;

        if (window.COLLECTIONS_CURRENT_INDEX >= allCollections.length) {
            if (nextCard) nextCard.style.display = 'none';
        } else {
            const nextCollection = allCollections[window.COLLECTIONS_CURRENT_INDEX];
            if (nextCard && nextCollection && nextCollection.cover_image) {
                nextCard.style.backgroundImage = `url('image-proxy.php?url=${encodeURIComponent(nextCollection.cover_image)}')`;
            }
        }

        const newCards = collectionsTrack.querySelectorAll('.collection-card.new-item');
        newCards.forEach(card => {
            setTimeout(() => card.classList.remove('new-item'), 400);
        });

        const isMobile = window.innerWidth < 768;
        const scrollMultiplier = isMobile ? 0.55 : 0.8;
        const scrollAmount = collectionsTrack.offsetWidth * scrollMultiplier;
        collectionsTrack.scrollBy({ left: scrollAmount, behavior: 'smooth' });

        const collectionsCarouselPrevBtn = document.querySelector('.carousel-btn-prev[data-carousel="collectionsTrack"]');
        const collectionsCarouselNextBtn = document.querySelector('.carousel-btn-next[data-carousel="collectionsTrack"]');
        ...
        function updateArrowsVisibility() {
            ...
            if (scrollLeft <= 0) {
                collectionsCarouselPrevBtn.style.opacity = '0';
                collectionsCarouselPrevBtn.style.pointerEvents = 'none';
            } else {
                collectionsCarouselPrevBtn.style.opacity = '';
                collectionsCarouselPrevBtn.style.pointerEvents = 'auto';
            }

            if (scrollLeft >= maxScroll - 5) {
                collectionsCarouselNextBtn.style.opacity = '0';
                collectionsCarouselNextBtn.style.pointerEvents = 'none';
            } else {
                collectionsCarouselNextBtn.style.opacity = '';
                collectionsCarouselNextBtn.style.pointerEvents = 'auto';
            }
        }
    ...
    function createCollectionCardHTML(collection, index) {
        const displayName = collection.display_name || collection.key;
        const movieCount = collection.movie_count || 0;
        const coverImage = collection.cover_image || '';
        const collectionKey = collection.key;

        let posterHTML = '';
        if (coverImage) {
            posterHTML = `<img src="image-proxy.php?url=${encodeURIComponent(coverImage)}" alt="${displayName}" loading="lazy" onerror="this.parentElement.innerHTML='<div class=\\'collection-placeholder\\'><svg width=\\'40\\' height=\\'40\\' viewBox=\\'0 0 24 24\\' fill=\\'none\\' stroke=\\'#666\\' stroke-width=\\'1.5\\'><rect x=\\'3\\' y=\\'3\\' width=\\'18\\' height=\\'18\\' rx=\\'2\\' ry=\\'2\\'/><circle cx=\\'8.5\\' cy=\\'8.5\\' r=\\'1.5\\'/><path d=\\'M21 15l-5-5L5 21\\'/></svg></div>'">`;
        } else {
            posterHTML = `<div class="collection-placeholder"><svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#666" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg></div>`;
        }

        return `
            <a href="collection.php?collection=${encodeURIComponent(collectionKey)}" class="collection-card new-item" data-collection-index="${index}">
                <div class="collection-poster">
                    ${posterHTML}
                </div>
                <div class="collection-info">
                    <div class="collection-text">
                        <h3 class="collection-title">${displayName}</h3>
                        <span class="collection-count">${movieCount} Movies</span>
                    </div>
    ...
    function showCollectionsContent() {
        const skeletonTrack = document.getElementById('collectionsSkeletonTrack');
        const actualTrack = document.getElementById('collectionsTrack');

        if (!skeletonTrack || !actualTrack) return;

        const images = actualTrack.querySelectorAll('.collection-poster img');
        const totalImages = images.length;

        function revealContent() {
            skeletonTrack.style.display = 'none';
            actualTrack.style.display = 'flex';
            initCollectionsCarousel();
            loadInitialCollectionsForDevice();
        }

        function loadInitialCollectionsForDevice() {
            if (!window.COLLECTIONS_DATA || typeof window.COLLECTIONS_CURRENT_INDEX === 'undefined') return;
            ...
            const currentCount = window.COLLECTIONS_CURRENT_INDEX;
            const additionalNeeded = targetCount - currentCount;
            
            if (additionalNeeded > 0 && currentCount < window.COLLECTIONS_DATA.length) {
                const collectionsTrack = document.getElementById('collectionsTrack');
                const nextCard = document.getElementById('collectionsNextCard');
                const allCollections = window.COLLECTIONS_DATA;
                
                const nextBatch = allCollections.slice(currentCount, currentCount + additionalNeeded);
                
                nextBatch.forEach((collection, idx) => {
                    const cardHTML = createCollectionCardHTML(collection, currentCount + idx);
                    if (nextCard) {
                        nextCard.insertAdjacentHTML('beforebegin', cardHTML);
                    } else {
                        collectionsTrack.insertAdjacentHTML('beforeend', cardHTML);
                ...
                window.COLLECTIONS_CURRENT_INDEX = currentCount + nextBatch.length;
        ...
        function checkAllPreloaded() {
            preloadedCount++;
            if (preloadedCount >= totalImages && !revealed) {
                revealed = true;
                revealContent();
            }
        }
        ...
        imageUrls.forEach(url => {
            ...
            preloadImg.onload = checkAllPreloaded;
            preloadImg.onerror = checkAllPreloaded;
            preloadImg.src = url;
        });
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function() {
            setTimeout(showCollectionsContent, 100);
        });
    } else {
        setTimeout(showCollectionsContent, 100);
    }
