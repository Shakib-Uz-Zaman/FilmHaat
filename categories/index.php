...
function fetchCategoryMovies($categoryKey, $websiteName, $website, $page = 1) {
    ...
    $results = parseCategoryResults($html, $website);
...
function parseCategoryResults($html, $website) {
    $results = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (empty($categoryKey) || !isset($CATEGORIES_WEBSITES[$categoryKey])) {
        echo json_encode([
            'success' => false,
    ...
    shuffle($allResults);
    $allResults = array_slice($allResults, 0, 12);
